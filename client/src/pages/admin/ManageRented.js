...
const ManageRented = () => {
    ...
    const fetchPosts = async (params) => {
        const response = location.pathname.includes('admin')
            ? await apiGetPostsByAdmin({ category: 'TNT', limit: process.env.REACT_APP_LIMIT_ADMIN, ...params })
            : await apiGetPosts({ category: 'TNT', limit: process.env.REACT_APP_LIMIT_ADMIN, ...params })
    ...
    return (
        <div className='relative h-full bg-white p-4'>
            ...
            <div className='flex items-center gap-8 border-b pb-4 justify-between'>
                ...
                <button
                    type='button'
                    onClick={() => navigate(`/${location.pathname.split('/')[1]}/${path.CREATE_POST}`, { state: { category: 'TNT' } })}
                ...
                <table className="table-auto w-full mt-4">
                    <thead>
                        <tr className='border-b border-t'>
                            ...
                            <td className='p-2 font-bold'>Giá</td>
                            <td className='p-2 font-bold'>Diện tích</td>
                    ...
                    <tbody>
                        {posts?.rows?.map((item, index) => (
                            ...
                            >
                                ...
                                <td className={`p-2 ${index % 2 === 0 ? '' : 'bg-gray-100'} m-auto`}>{item?.area}</td>
