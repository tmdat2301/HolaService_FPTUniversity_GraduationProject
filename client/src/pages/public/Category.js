...
import { Post, Pagination, SidebarElem, Filter, RelatedPost } from '../../components'
...
const Category = () => {
    ...
    return (
        <div className='w-full flex flex-col gap-3 relative'>
            <div>
                <h1 className='text-[28px] font-bold' >{categories?.find(el => el.slug === category)?.header}</h1>
                <p className='text-base text-gray-700'>{categories?.find(el => el.slug === category)?.subheader}</p>
            </div>
            ...
            <div className='flex gap-4'>
                ...
                <div className='flex-auto w-[30%] flex flex-col gap-4'>
                    <SidebarElem
                        ...
                        content={categories}
                    />
                    {category === 'Thue-nha-tro' && <>
                        <SidebarElem
                            category={categories?.find(el => el.slug === category)?.code}
                            title='Xem theo giá'
                            content={prices}
                            isDouble
                            type='price'
                    ...
                    <SidebarElem
                        ...
                        type='distance'
                    />
                    <RelatedPost cate={categories?.find(el => el.slug === category)?.code} />
                </div>
            </div>
